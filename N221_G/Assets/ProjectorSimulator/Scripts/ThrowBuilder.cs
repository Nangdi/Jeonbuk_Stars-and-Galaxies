...
public class ThrowBuilder : MonoBehaviour
{
    ...
    public void BuildThrow(float throwRatio, float aspect, Vector2 shift_percentage, float maxDistance, bool stopAtCollision, Material throwMaterial)
    {
        ...
        if (maxDistance < 0.1f)
            maxDistance = 0.1f;
        float imageWidth = maxDistance / throwRatio;
        ...
        vertices[1] = new Vector3(rightEdge - imageWidth, topEdge              , maxDistance);
        vertices[2] = new Vector3(rightEdge             , topEdge              , maxDistance);
        vertices[3] = new Vector3(rightEdge             , topEdge - imageHeight, maxDistance);
        vertices[4] = new Vector3(rightEdge - imageWidth, topEdge - imageHeight, maxDistance);
        ...
        if (stopAtCollision)
        {
            ...
            {
                // cast a ray from the lens centre through the temporary vertex - if we hit something, use the largest distance
                float angle = Vector3.Angle(Vector3.forward, vertices[i].normalized);
                RaycastHit[] hits = Physics.RaycastAll(transform.position, transform.localToWorldMatrix.MultiplyPoint(vertices[i]) - transform.position, vertices[i].magnitude);
